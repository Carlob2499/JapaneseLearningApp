...
- **Verification pass:** after drafting, an adversarial re-check re-fetched 18 sampled load-bearing claims on 2026-07-11 (official quotes, counts tables, government survey percentages, learning-science caveats): 17 supported — most verbatim, including the guidebook FAQ Q7 quote (extracted from the official PDF) and both moj.go.jp survey pages — and 1 citation misattribution found and repaired (§4 M1: the 33.8% gairaigo figure re-cited to its actual sources).
...
**Built (D-016):** the grammar layer now ships as a first-class `curated-cited` content type across all five levels — a genuinely-cited core per level (L1 25 · L2 24 · L3 19 · L4 16 · L5 14 = **98 points**, growing toward the ~848 total above). Each point's name/gloss/summary are original prose with ≥2 inventory citations (JLPT Sensei per-level + Bunpro) and, for N5/N4, a verified Genki I/II chapter anchor. Crucially, **example sentences are drawn verbatim from Tatoeba** at build time by pattern-matching (never authored — D-002); a point that finds no readable example at its level is held back, so every shipped point carries real, attributed examples.
...
- https://user.keio.ac.jp/~rhotta/hellog/2013-10-28-1.html — gairaigo 9.8% of distinct words (1956 NINJAL magazine corpus)
- https://ja.wikipedia.org/wiki/%E8%AA%9E%E7%A8%AE + https://www.anlp.jp/proceedings/annual_meeting/2004/pdf_dir/P6-3.pdf — gairaigo 33.8% of distinct words, 1994 NINJAL magazine survey (35.8% before category exclusions)
